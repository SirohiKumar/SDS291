...
## Section 3.6: Testing Subsets of Predictors (03-14-24)

we can make `anova()` perform a nested F-test by supplying it multiple models
...
```
we prefer to load these models in ascending order of complexity (fewest --> most number of predictors) in order to ensure we don't have negative sums of square and dfs. 

#### 3 or more models
3 or more models will allow R to perform nested F-tests in pairs. generate one more model
```{r}
...
row 2 compares model 2 (`price_size_beds_lot`) with model 1 (`price_size`), and row 3 compares model 3 (`price_all_variables`) to model 2 (`price_size_beds_lot`). 

#### possible errors

if you supply the `anova()` with non-nested models, you *can* get missing F-statistics and p-values: 
```{r}
...
```
however, R doesn't always protect us, so remember the conditions under which these models operate: 
```{r}

price_beds = lm(Price ~ Beds, data = HousesNY)
price_size_lot = lm(Price ~ Size + Lot, data = HousesNY)
anova(price_beds, price_size_lot)
